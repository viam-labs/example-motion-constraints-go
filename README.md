...
All four constraint variants below are empirically verified to plan reliably from the `homeJointPositionsReady` start pose at the Y±100mm/Z=450 anchors with uniform gripper offsets — see `cmd/probe/main.go` for the parameter-sweep tool that generated these settings.

| Arm | Constraint | What it does | Known wart |
| --- | --- | --- | --- |
| `arm_a1` | None (baseline) | cbirrt picks a natural joint-space path between anchors — usually a curve in cartesian space because joint-space shortest path doesn't map to cartesian-space shortest. | None — this is the reference. |
| `arm_a2` | `LinearConstraint{LineToleranceMm: 50, OrientationToleranceDegs: 180}` | EE position is forced inside a tight 50mm tube along the straight line between anchors. Orientation is unconstrained. | The planner internally generates an intermediate waypoint every 10mm of cartesian motion under a LinearConstraint (see `motionplan/armplanning/plan_manager.go::generateWaypoints` — `defaultStepSizeMM=10`). Each waypoint becomes its own IK + cbirrt sub-plan in the budget. A 200mm swing → 20 sub-plans (works); a 400mm swing → 40 sub-plans (frequently times out). The tolerance values don't change the step count, only step size at very tight tolerances. Trajectories also come back ~5–10× denser than unconstrained, which is why the module caps preview-ghost emissions via `max_preview_ghosts`. |
| `arm_a3` | `OrientationConstraint{OrientationToleranceDegs: 90}` | EE orientation stays within 90° of the interpolated path orientation. Position is free to take any path. | Tight tolerances (≤45°) fail when the home pose's wrist orientation is far from the goal orientation — `cbirrt` can't bridge the gap in one shot. 90° is the empirical "just loose enough" for the ready-pose-to-identity-goal transition; 60° is borderline (sometimes works, sometimes timeouts); 45° consistently fails. |
| `arm_a4` | Combined: `LinearConstraint{LineToleranceMm: 200, OrientationToleranceDegs: 45}` | Both: position inside a 200mm tube AND orientation within 45° along the path. | Combined constraints multiply rather than add in difficulty. With Linear alone, position tube of 50mm works; with combined, position must be looser (200mm) AND orientation looser (45° with `LinearConstraint`'s min-of-endpoints check, which is more permissive than `OrientationConstraint`'s linear interpolation check). Tighter combined parameters (50mm + 45°) consistently fail. |

The pedagogical pattern: each arm shows one constraint class working. The README documents the tighter-tolerance variants that *don't* work so users know where the cliff is. If you want a "constraint that fails by design" demo, switch any arm to `random_translation_linear` (which uses workspace-spanning waypoints under a constraint and frequently times out) — that's the "what failure looks like" preset.
